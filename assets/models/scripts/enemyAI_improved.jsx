...
export class EnemyAI {
    constructor(scene, world, camera, characterBody, RAPIER, audioListener = null, spawnPosition = null) {
        ...
        this.attackData = {
            ...
            knockbackStrength: 200,    // How hard to push player (strong knockback)
    ...
    setupAudio() {
        // Ensure we don't accidentally create duplicate audio nodes (HMR / re-init)
        try {
            if (this.audio && this.audio.footstepSound) {
                this.audio.footstepSound.stop && this.audio.footstepSound.stop();
                if (this.mesh && this.audio.footstepSound.parent) {
                    this.audio.footstepSound.parent.remove(this.audio.footstepSound);
                }
            }
            if (this.audio && this.audio.wailingSound) {
                this.audio.wailingSound.stop && this.audio.wailingSound.stop();
                if (this.mesh && this.audio.wailingSound.parent) {
                    this.audio.wailingSound.parent.remove(this.audio.wailingSound);
                }
            }
            if (this.alertSound) {
                this.alertSound.stop && this.alertSound.stop();
                if (this.mesh && this.alertSound.parent) {
                    this.alertSound.parent.remove(this.alertSound);
                }
            }
        } catch (e) {
            console.warn('Error cleaning prior audio nodes:', e);
        }
        ...
        const audioLoader = new THREE.AudioLoader();
                ...
                audioLoader.load('/assets/sounds/fx/heavy-walking-footsteps.mp3', (buffer) => {
                    ...
                    this.audio.footstepSound.setVolume(3.2); // Louder footsteps
            ...
            // Small helper to avoid overlapping footstep playback
            try { this.audio._lastFootstepPlayTime = -9999; } catch (e) {}
    ...
    loadSkeletonModel() {
        ...
        loader.load('/assets/models/Skeleton/Skeleton.gltf', (gltf) => {
            ...
            this.skeletonModel.scale.set(9.6, 9.6, 9.6);
            this.skeletonModel.position.y = -12; // Ground level for crawling
    ...
    addGlowingEyes() {
        ...
        if (headBone) {
            ...
            leftEye.position.set(0.03, 0.09, 0.09);
            rightEye.position.set(-0.03, 0.09, 0.09);
    ...
    changeState(newState) {
        ...
        if (newState === this.states.ALERT) {
            ...
            // (No automatic growl here; alert sound plays during ALERT)
            ...
            this.timers.search = 0;
            // Prevent immediate overlapping footsteps when switching from CHASE->SEARCH
            try {
                if (this.audio && this.audio.footstepSound && this.audio.footstepSound.isPlaying) {
                    this.audio.footstepSound.stop && this.audio.footstepSound.stop();
                }
                const now = (typeof performance !== 'undefined' ? performance.now() : Date.now()) / 1000;
                if (this.audio) this.audio._lastFootstepPlayTime = now;
        ...
        if (isMoving && footstepInterval > 0) {
            ...
            if (this.audio.lastFootstep > footstepInterval) {
                if (this.audio.footstepSound && this.audio.footstepSound.buffer) {
                    ...
                    // Throttle rapid replays: require a small gap between actual play calls
                    try {
                        const now = (typeof performance !== 'undefined' ? performance.now() : Date.now()) / 1000;
                        const last = (this.audio && this.audio._lastFootstepPlayTime) || -9999;
                        if ((now - last) >= 0.075) {
                            try {
                                if (this.audio.footstepSound.isPlaying) {
                                    this.audio.footstepSound.stop();
                                }
                            } catch (e) {}
                        ...
                        // Fallback to naive play on error
                        try { this.audio.footstepSound.play(); } catch (e) {}
                    }
                }
                this.audio.lastFootstep = 0;
            }
        ...
        try {
            if (this.audio) {
                if (this.audio.wailingSound) {
                    this.audio.wailingSound.stop && this.audio.wailingSound.stop();
                    if (this.mesh && this.audio.wailingSound.parent) this.audio.wailingSound.parent.remove(this.audio.wailingSound);
                }
                if (this.audio.footstepSound) {
                    this.audio.footstepSound.stop && this.audio.footstepSound.stop();
                    if (this.mesh && this.audio.footstepSound.parent) this.audio.footstepSound.parent.remove(this.audio.footstepSound);
                }
                if (this.alertSound) {
                    this.alertSound.stop && this.alertSound.stop();
                    if (this.mesh && this.alertSound.parent) this.alertSound.parent.remove(this.alertSound);
                }
                if (this.audio.tensionSound) {
                    this.audio.tensionSound.stop && this.audio.tensionSound.stop();
                }
                // Reset audio flags
                this.audio.isWailingPlaying = false;
                this.audio.isTensionPlaying = false;
                this.audio.footstepLoaded = false;
            }
        } catch (e) {
            console.warn('Error during audio cleanup in destroy():', e);
        }
